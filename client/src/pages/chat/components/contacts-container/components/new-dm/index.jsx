    ...
    <div>
      <TooltipProvider>
        <Tooltip>
          ...
          <TooltipContent side="top" className="bg-[#67666b] border border-white/10 text-white text-xs px-3 py-1.5 rounded-lg">
            New Direct Message
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
...
<div className="contacts-scroll h-[260px] overflow-y-auto">
                    ...
                    >
                      <Avatar className="w-9 h-9 rounded-full overflow-hidden block shrink-0">
                        {contact.image ? (
                          ...
                          <div className={`uppercase text-sm font-semibold flex items-center justify-center w-full h-full ${getColor(contact.color)}`}>
                            {getInitial(contact)}
                          </div>
                        )}
                      </Avatar>
                      ...
                      <span className="text-white/0 group-hover:text-white/20 text-xs transition-colors shrink-0">›</span>
                    </button>
                  ))}
                </div>
              )}
              ...
              {!loading && searchedContacts.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full gap-2">
                  ...
                  <p className="text-white/15 text-xs">Search above to find someone</p>
                </div>
              )}
